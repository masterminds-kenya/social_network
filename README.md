...
For Instagram professional accounts, the expectation is that it is attached to a professional page on Facebook, as is customary. In order for our platform to get the most accurate metrics on that user's Stories, their Facebook page needs to have the App platform enabled in their [App Settings](https://www.facebook.com/settings?tab=applications). In general, the accurate recording of a user's posts and metrics depends on that user granting, and continuing to allow, the permissions as requested when onboarding to the platform.
