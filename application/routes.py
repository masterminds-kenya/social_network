...
def update_campaign_metrics(id):
    ...
    count, success, ignore = media_posts_save(post_data, bulk_db_operation='update')
    if success:
...
def all_posts():
    ...
    if num_users > MAX_ACTIVE_USER_NUM:
        ...
        count, success, db_response = media_posts_save(media_results, return_ids=True, add_time=True)
...
def collect_queue(mod, process):
    ...
    if isinstance(result, list):
        count, success, ignore = media_posts_save(result, bulk_db_operation='update')
        if success:
...
def new_post(mod, id):
    ...
    count, success, ignore = media_posts_save(media_results, add_time=False)
